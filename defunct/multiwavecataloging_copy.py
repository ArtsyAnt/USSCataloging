...
class CatalogOverlayer:
    ...
    def is_in_ellipse(self):
        ...
        for index in range(self.comparision_index):
            ...
            for iteration in range(len(self.base.catalog[self.base.ra])):
                ...
                # 2. defining ra as x and dec as y
                '''check removing all the degress that were incorrectly put on'''
                x_distances_from_center = ra_points  - ra_center_ellipses[iteration]
                y_distances_from_center = dec_points - dec_center_ellipses[iteration]
                
                # 3. define angles since we are using it reverse should be reversed
                theta = np.radians(position_angle[iteration])
                cos = np.cos(theta)
                sin = np.sin(theta)
    ...
    def ellipse_point_indexing_V2(self):
        ...
...
def spectral_window_merging_V2(base_window, overlapping_windows, matched_dict): 
    ...
    # iterate through the spectral windows 
    # for spw in Spectral_window_types:
    #     spw_cat = overlapping_windows[overlapping_windows['spw_id'] == spw]
    ...
    # might be better to leave it here and then apply a code that uses this to loop through the unique findings
...
#     ra_ellipse = coord.Angle(self.base.catalog[self.base.ra])
#     ra_ellipse = - ra_ellipse
#     ra_reordered_ellipse = ra_ellipse.wrap_at(180*u.deg)
#     dec_ellipse = coord.Angle(self.base.catalog[self.base.ra])
...
#     for i in range(len(self.base.catalog[self.base.ra])):
#             ellipses_patch = ellipse((ra_reordered_ellipse[i].value, dec_ellipse[i].value),  self.base.semi_major_ax[i].value*2,  self.base.semi_minor_ax[i].value*2, angle=self.base.position_angle[i].value, lw=2, edgecolor='b', alpha=0.5, zorder=1)
#             self.ellipse_patches.append(ellipses_patch)
